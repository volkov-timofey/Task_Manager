...
class BaseTest(TestCase):
    ...
    def test_tasks_create_view(self):
        ...
        self.assertIn('Задача успешно создана', content)
    ...
    def test_update_task(self):
        ...
        response = self.client.get('/tasks/')
        content = response.content.decode()
        self.assertIn('Задача успешно изменена', content)
        self.assertIn('task_unaction', content)
        self.assertIn('Задачи', content)
    ...
    def test_delete_task(self):
        ...
        response = self.client.get('/tasks/')
        content = response.content.decode()
        self.assertIn('Задачу может удалить только ее автор', content)
        self.assertRedirects(response_redirect, '/tasks/', 302, 200)
        ...
        response = self.client.get('/tasks/')
        content = response.content.decode()
        self.assertIn('Задача успешно удалена', content)
